...
type KanbanState = {
  ...
  addTask: (columnKey: string, title: string, description?: string) => void;
...
export const useTaskStore = create<KanbanState>()(
  ...
  (set) => ({
    ...
    addTask: (columnKey, title, description) =>
      set((state) => ({
        columns: {
          ...state.columns,
          [columnKey]: [
    ...
    updateTask: (taskId, patch) =>
      set((state) => {
        ...
        for (const [key, tasks] of Object.entries(state.columns)) {
          columns[key] = tasks.map((task) => (task.id === taskId ? { ...task, ...patch } : task));
        }
        return { columns };
      })
  })
