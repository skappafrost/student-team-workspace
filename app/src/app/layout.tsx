...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Persisted dark/light mode (mirrored to a cookie client-side) so the
  // server renders <html class="dark|light"> correctly on first paint.
  const modeCookie = cookieStore.get('theme')?.value;
  const defaultMode = modeCookie === 'dark' || modeCookie === 'light' ? modeCookie : 'dark';
    ...
    <html lang='en' suppressHydrationWarning data-theme={themeToApply}>
      ...
      >
        ...
        <NuqsAdapter>
          <ThemeProvider
            attribute='class'
            // D4: dark default on fresh loads; cookie restores saved mode
